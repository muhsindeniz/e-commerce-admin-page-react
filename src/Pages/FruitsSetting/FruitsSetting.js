...
const FruitsSetting = () => {
  ...
  const onFinish = async () => {
    if (productData.name == "" || productData.discount == "" || productData.price == "" || productData.newPrice == "" || productData.productDescription == "" || productData.farmerName == "" || productData.calorie == "" || productData.carbohydrate == "" || productData.oil == "", productData.stock == "") {
      ...
      await axios.post(url, formData, config).then(resp => {
        ...
        axios.post('http://localhost:3000/api/addFruit', {
  ...
  let call = useMemo(() => async () => {
    setLoading(true)
    if (token) {
      await axios.get('http://localhost:3000/api/fruit', {
        headers: {
          Authorization: token
        }
      })
  ...
  let deleteUser = async (id) => {
    setLoading(true)
    if (id) {
      await axios.delete(`http://localhost:3000/api/fruit/${id}`, {
        headers: {
          Authorization: token
        }
      })
  ...
  let onEditDataProduct = async () => {
    if (productData.name == "" || productData.discount == "" || productData.price == "" || productData.newPrice == "" || productData.productDescription == "" || productData.farmerName == "" || productData.calorie == "" || productData.carbohydrate == "" || productData.oil == "", productData.stock == "") {
      ...
      if (imageData === null) {
        axios.patch(`http://localhost:3000/api/fruit/${proId}`, {
          name: productData.name,
          price: productData.price,
          discount: productData.discount,
          newPrice: productData.newPrice,
